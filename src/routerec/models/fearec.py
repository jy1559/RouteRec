...
class FEARec(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        self.need_semantic_aug = bool(
            self.ssl in ["us", "su", "us_x"]
            and (float(self.lmd_sem) > 0.0 or (self.ssl == "us_x" and bool(self.fredom)))
        )
    ...
    def get_same_item_index(self, dataset):
        ...
        for index, item_id in enumerate(target_item_np.tolist()):
            key = int(item_id)
            if key not in same_target_index:
                same_target_index[key] = []
            same_target_index[key].append(int(index))
    ...
    def _sample_semantic_positive_batch(self, interaction):
        ...
        for i, item_id in enumerate(target_items_cpu):
            candidates = self.same_item_index.get(int(item_id))
    ...
    def calculate_loss(self, interaction):
        ...
        if self.need_semantic_aug:
            if "sem_aug" in interaction.interaction and "sem_aug_lengths" in interaction.interaction:
                ...
                interaction.update(Interaction({"sem_aug": sem_aug, "sem_aug_lengths": sem_aug_lengths}))
        ...
        # Supervised NCE
        if self.ssl in ["us", "su"] and sem_aug is not None and float(self.lmd_sem) > 0.0:
        ...
        if self.ssl == "us_x" and sem_aug is not None:
            aug_seq_output = self.forward(item_seq, item_seq_len)
            sem_aug_seq_output = self.forward(sem_aug, sem_aug_lengths)
            sem_nce_logits, sem_nce_labels = self.info_nce(
                aug_seq_output,
    ...
    def info_nce(self, z_i, z_j, temp, batch_size, sim="dot"):
        ...
        if batch_size != self.batch_size:
            ...
            if self.mask_default.device != sim.device:
                self.mask_default = self.mask_default.to(sim.device)
...
class HybridAttention(nn.Module):
    ...
    def __init__(
        ...
        self.n_layers = max(1, int(config["n_layers"]))
        self.verbose = bool(config["fearec_verbose"]) if "fearec_verbose" in config else False
        layer_ratio = 1 / self.n_layers
        if self.global_ratio > layer_ratio:
            if self.verbose:
                print(
                    "{}>{}:{}".format(
                        self.global_ratio,
                        layer_ratio,
                        self.global_ratio > layer_ratio,
                    )
                )
            self.filter_mixer = "G"
        else:
        ...
        max_freq_bins = self.max_item_list_length // 2 + 1
        self.left = max(0, min(self.left, max_freq_bins - 1))
        self.right = max(self.left + 1, min(self.right, max_freq_bins))
