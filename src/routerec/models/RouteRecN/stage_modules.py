"""Lightweight stage modules for RouteRecN."""
...
from ..RouteRecBase.moe_stages import MoEStage as LegacyMoEStage
from ..RouteRecBase.moe_stages import _scaled_expert_lists, _scaled_expert_names
from ..RouteRecBase.routers import Router, RuleSoftRouter
from ..RouteRecBase.transformer import PositionwiseFFN, TransformerEncoder
from .feature_bank import _to_ratio
from ..RouteRecV2.feature_config import STAGE_ALL_FEATURES
...
class NMoEStage(nn.Module):
    ...
    def __init__(self, cfg: StageRuntimeConfig):
        ...
        if variant != "ratio_bins":
            raise ValueError(
                "RouteRecN only supports rule_router.variant='ratio_bins'. "
                f"Got {variant}"
            )
        ...
        if router_mode not in {"token", "session"}:
            raise ValueError(f"Unsupported router mode for RouteRecN stage: {router_mode}")
        if session_pooling not in {"mean", "last"}:
            raise ValueError(
                "RouteRecN supports macro_session_pooling in ['mean','last'] only. "
...
class StageBranchRunner(nn.Module):
    """Run one explicit pass/MoE branch for RouteRecN."""
