...
from .backbone_stages import _normalize_top_k, _softmax_with_top_k
...
from .routers import Router, RuleSoftRouter, build_group_local_stat_logits_12way
from .routing import (
...
from .sparse_stages import _scaled_expert_names
...
class RouteRecStage(nn.Module):
    """Shared per-stage FFN/MoE block used by layer_layout stage tokens."""

    def __init__(self, cfg: RouteRecStageConfig):
        super().__init__()
        ...
        self.routing_control_mode = normalize_control_mode(cfg.routing_control_mode)
        ...
        if self.routing_control_mode == "random_frozen":
    ...
    def _compute_router_outputs(
        ...
        *,
        ...
        if self.stage_router_mode == "rule_soft":
            ...
            encoded_for_router = controlled_feature_input(encoded_feat, mode=self.routing_control_mode)
            raw_for_router = controlled_feature_input(stage_raw_feat, mode=self.routing_control_mode)
            if self.routing_control_mode == "hidden_only_matched":
            ...
            hidden_for_router = controlled_hidden_input(hidden, mode=self.routing_control_mode)
