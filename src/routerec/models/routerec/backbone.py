"""Shared sequential backbone used by the public RouteRec model."""
...
from .config import ConfigResolver, parse_layout_catalog_from_config
from .layout import (
    LayoutSpec,
...
from .losses import compute_expert_aux_loss, compute_stage_merge_aux_loss
...
from .backbone_executor import BackboneStageExecutor
...
class RouteRecBackbone(SequentialRecommender):
    """Sequential backbone with shared feature and routing infrastructure."""
    ...
    def __init__(self, config, dataset):
        ...
        resolver.assert_unsupported_keys()
        resolver.assert_embedding_only_dimension()
        if not _is_none_like(resolver.get("teacher_design", None)):
            raise ValueError("RouteRec does not support 'teacher_design'.")
        if not _is_none_like(resolver.get("teacher_delivery", None)):
            raise ValueError("RouteRec does not support 'teacher_delivery'.")
        if bool(resolver.get("router_distill_enable", False)):
            raise ValueError("RouteRec does not support router_distill_enable.")
        if bool(resolver.get("feature_specialization_aux_enable", False)):
            raise ValueError("RouteRec does not support feature specialization auxiliary loss.")
        ...
        self.layout_id = int(resolver.get("layout_id", 0))
        if not (0 <= self.layout_id < len(self.layout_catalog)):
            raise ValueError(
                f"layout_id out of range: id={self.layout_id}, catalog_size={len(self.layout_catalog)}"
            )
        selected_layout = self.layout_catalog[self.layout_id]
        execution_override = str(
            resolver.get("stage_execution_mode", selected_layout.execution)
        ).lower().strip()
        if execution_override not in {"serial", "parallel"}:
            raise ValueError(
                f"stage_execution_mode must be one of ['serial','parallel'], got {execution_override}"
            )
        ...
        if rule_variant != "ratio_bins":
            raise ValueError(
                "RouteRec supports rule_router.variant='ratio_bins' only."
            )

        self.router_design = str(resolver.get("router_design", "simple_flat")).lower().strip()
        if self.router_design != "simple_flat":
            raise ValueError("RouteRec only supports router_design='simple_flat'.")
        ...
        self.parallel_stage_gate_top_k = int(resolver.get("parallel_stage_gate_top_k", 0))
        ...
        self.parallel_stage_gate_temperature = float(
            resolver.get("parallel_stage_gate_temperature", 1.0)
        )
        ...
        self.stage_merge_aux_enable = bool(resolver.get("stage_merge_aux_enable", False))
        self.stage_merge_aux_lambda_scale = float(
            resolver.get("stage_merge_aux_lambda_scale", 1.0)
        )
        ...
        self.stage_executor = BackboneStageExecutor(
            layout=self.layout,
            d_model=self.d_model,
        ...
        try:
            config["layout_id"] = int(self.layout_id)
            config["stage_execution_mode"] = self.layout.execution
            config["layout_summary"] = stage_boundary_summary(self.layout)
        ...
        logger.info(
            "RouteRec backbone: layout_id=%s execution=%s boundaries=%s total_moe_blocks=%s "
