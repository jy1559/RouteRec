"""Feature configuration and metadata helpers for RouteRec."""
...
from ...datasets import find_feature_metadata, resolve_dataset_dir
...
def load_feature_metadata(*, data_path: str, dataset: str) -> dict:
    dataset_dir = resolve_dataset_dir(dataset=dataset, data_path=data_path)
    candidates: list[Path] = []
    if dataset_dir is not None:
        resolved = find_feature_metadata(dataset_dir)
        if resolved is not None:
            candidates.append(resolved)

    data_root = Path(str(data_path))
    for candidate_root in (data_root / str(dataset), data_root):
        resolved = find_feature_metadata(candidate_root)
        if resolved is not None:
            candidates.append(resolved)
...
def validate_feature_metadata(meta: dict) -> dict:
    ...
    if not {5, 10}.issubset(ok_windows):
        raise ValueError(f"feature metadata macro_windows must contain 5 and 10, got {windows}")
    for family_name in ("macro5", "macro10", "mid", "micro"):
        groups = families.get(family_name, {})
        if not isinstance(groups, dict):
            raise ValueError(f"feature metadata families.{family_name} missing or invalid")
        for group_name in GROUP_ORDER:
            if group_name not in groups:
                raise ValueError(
                    f"feature metadata families.{family_name} missing group {group_name}"
                )
    return meta
