"""Configuration resolution and validation for RouteRec."""
...
# Unsupported historical keys are rejected rather than silently ignored.
_UNSUPPORTED_KEYS = {
...
class ConfigResolver:
    ...
    def require(self, key: str) -> Any:
        if not self.has(key):
            raise ValueError(f"RouteRec requires config key '{key}'")
        return self.get(key)

    def assert_unsupported_keys(self) -> None:
        found = [k for k in sorted(_UNSUPPORTED_KEYS) if self.has(k)]
        if found:
            joined = ", ".join(found)
            raise ValueError(
                "RouteRec does not support these historical config keys: "
                f"{joined}. Use the public layout and schedule keys instead."
            )
    ...
    def assert_embedding_only_dimension(self) -> None:
        ...
        if self.has("hidden_size"):
            ...
            if h != e:
                raise ValueError(
                    "RouteRec uses embedding_size as the single dimension key. "
...
def parse_layout_catalog_from_config(resolver: ConfigResolver) -> Iterable[Dict[str, Any]]:
    catalog = resolver.get("layout_catalog", None)
    if catalog is None:
        # Default fallback catalog (single serial layout).
        return [
            {
                "id": "L0",
                "execution": resolver.get("stage_execution_mode", "serial"),
                "global_pre_layers": 1,
