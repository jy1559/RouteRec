"""RouteRec model with an explicit layer-layout backbone."""
...
from .backbone import RouteRecBackbone
from .config import ConfigResolver
from .diagnostics import RoutingDiagnosticCollector
...
from .executor import RouteRecStageExecutor
...
class RouteRec(RouteRecBackbone):
    """Behavior-guided sparse router with an explicit layer layout."""
    ...
    def _ensure_layout_catalog(config) -> None:
        ...
        _set_config_key(config, "layout_catalog", copy.deepcopy(catalog))
        _set_config_key(config, "layout_id", 0)
        _set_config_key(config, "stage_execution_mode", "serial")
    ...
    def __init__(self, config, dataset):
        ...
        self.routing_control_mode = str(
            resolver.get("routing_control_mode", "full")
        ).lower().strip()
        ...
        self._diag_collector: Optional[RoutingDiagnosticCollector] = None

        meta = load_feature_metadata(
            data_path=str(resolver.get("data_path", "")),
            dataset=str(resolver.get("dataset", "")),
        )
        self.feature_metadata = validate_feature_metadata(meta) if meta else {}
        ...
        # If feature metadata is unavailable, disable engineered columns only when
        # the caller explicitly requests a feature-free run.
        # disable engineered feature columns for this run to avoid noisy missing-field fallbacks.
        meta_feature_list = list((self.feature_metadata or {}).get("all_features", []) or [])
        if meta_feature_list:
            ...
            if requested_feature_mode == "none":
                effective_feature_columns = []
                logger.warning(
                    "Engineered feature columns are disabled for dataset=%s "
                    "because feature_mode=%s.",
        ...
        self.stage_executor = RouteRecStageExecutor(
            ...
            routing_control_mode=self.routing_control_mode,
    ...
    def begin_diagnostic_eval(self, *, split_name: str) -> None:
        if not self.routerec_diag_logging or not self._supports_diag:
            ...
            return
        self._diag_collector = RoutingDiagnosticCollector(
