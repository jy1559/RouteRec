...
def _parse_stage(stage_name: str, payload: Any) -> StageLayoutSpec:
    ...
    if not isinstance(payload, dict):
        raise ValueError(
            f"layout_catalog[*].stages.{stage_name} must be object, got {type(payload).__name__}"
        )
...
def parse_layout_catalog(raw_catalog: Any) -> List[LayoutSpec]:
    """Parse and validate the RouteRec layout catalog.
    ...
    if not isinstance(raw_catalog, Iterable) or isinstance(raw_catalog, (str, bytes, dict)):
        raise ValueError("layout_catalog must be a non-empty list of layout objects")

    parsed: List[LayoutSpec] = []
    for idx, item in enumerate(raw_catalog):
        if not isinstance(item, dict):
            raise ValueError(
                f"layout_catalog[{idx}] must be object, got {type(item).__name__}"
            )
        ...
        if execution not in _ALLOWED_EXECUTION:
            raise ValueError(
                f"layout_catalog[{idx}].execution must be one of {sorted(_ALLOWED_EXECUTION)}, "
                f"got {execution}"
            )

        global_pre_layers = _to_non_negative_int(
            item.get("global_pre_layers", 0), f"layout_catalog[{idx}].global_pre_layers"
        )
        global_post_layers = _to_non_negative_int(
            item.get("global_post_layers", 0), f"layout_catalog[{idx}].global_post_layers"
        )

        raw_stages = item.get("stages", {})
        if not isinstance(raw_stages, dict):
            raise ValueError(
                f"layout_catalog[{idx}].stages must be object, got {type(raw_stages).__name__}"
            )
        ...
        if total_layers > _MAX_TOTAL_ATTN_LAYERS:
            raise ValueError(
                f"layout_catalog[{idx}] total_layers={total_layers} exceeds "
    ...
    if not parsed:
        raise ValueError("layout_catalog must be non-empty")
    return parsed


def active_stage_names(layout: LayoutSpec) -> List[str]:
    """Return stages that have any stage-local compute."""
    out: List[str] = []
    for stage_name in _STAGE_NAMES:
