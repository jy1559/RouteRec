...
class RouteRecStageExecutor(nn.Module):
    """Execute explicit RouteRec layer-layout tokens from left to right."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.routing_control_mode = normalize_control_mode(routing_control_mode)
        ...
        for stage_name in _STAGE_NAMES:
            if stage_name not in self.stage_blocks:
                ...
                cfg = RouteRecStageConfig(
                    ...
                    routing_control_mode=str(routing_control_mode),
                ...
                self.stage_blocks[stage_name] = RouteRecStage(cfg)
        ...
        *,
        ...
        if agg == "router":
            ...
            weights, logits = self.bundle_router_modules[router_index](
                hidden=controlled_hidden_input(base_hidden, mode=self.routing_control_mode),
                feat=controlled_feature_input(feat_in, mode=self.routing_control_mode),
            )
            merged = base_hidden + (weights.unsqueeze(-1) * deltas).sum(dim=-2)
            return merged, weights, logits
    ...
    def forward(
        ...
        *,
        ...
        out = hidden
        # Apply the routing intervention once at the executor boundary so no
        ...
        controlled_feat = (
            None
            if feat is None
            else controlled_feature_input(feat, mode=self.routing_control_mode)
        )
