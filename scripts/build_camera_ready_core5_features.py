#!/usr/bin/env python3
"""Build, independently validate, and atomically publish camera-ready features.

Each dataset is built into a hidden same-filesystem directory.  The existing
full-v5 builder consumes the frozen basic split, recomputes every one of the
...
def tree_manifest(directory: Path) -> dict[str, object]:
    ...
    return {
        "schema_version": 1,
        "contract": "camera-ready-core5-full-v5-tree-manifest-v1",
        "files": files,
...
def build_one(
    *,
    ...
    builder = repo_root / "scripts" / "build_full_feature_dataset.py"
    validator = repo_root / "scripts" / "validate_full_feature_dataset.py"
    if not builder.is_file() or not validator.is_file():
        raise FileNotFoundError("full-v5 builder or validator is missing")
    try:
        build_command = [
            sys.executable,
            "-u",
        ...
        if "--overwrite" in build_command:
            raise AssertionError("camera-ready feature build must never pass --overwrite")
        ...
        manifest_path = staged_dir / f"{dataset}.camera_ready_manifest.json"
        ...
        publication = {
            "schema_version": 1,
            "contract": "camera-ready-core5-full-v5-publication-v1",
            "dataset": dataset,
