...
FEATURE_CONTRACT = "full-v5-leakage-correct-20260812"
METADATA_CONTRACT = "full-v5-location-independent-metadata-v1"
BASE_HEADER = (
    "session_id:token",
    "item_id:token",
    "timestamp:float",
...
def validate_metadata(
    *,
    ...
    is_camera_ready_basic = (
        basic_summary.get("camera_ready_contract")
        == "camera-ready-core5-parent-strict-v1"
    )
    if is_camera_ready_basic:
        if basic_summary.get("status") not in (
            "complete",
            "complete_staged_pending_independent_validation",
        ):
            errors.add(
                "camera-ready basic status is neither complete nor staged-complete"
            )
    else:
    ...
    require_equal(
        errors, feature_meta.get("reconstruction_contract"), FEATURE_CONTRACT,
        "feature reconstruction contract",
    )
    require_equal(
    ...
    if not isinstance(canonical, dict):
        ...
        require_equal(errors, canonical.get("contract"), METADATA_CONTRACT, "metadata contract")
    ...
    require_equal(errors, build_summary.get("feature_contract_version"), FEATURE_CONTRACT, "build contract")
    ...
    *,
    ...
    feature_paths = {
        ...
        **{split: feature_dir / f"{target_dataset}.{split}.inter" for split in SPLITS},
        ...
        "meta": feature_dir / "feature_meta_v3.json",
    ...
    if declared_max is None and is_camera_ready_basic:
    ...
    if not isinstance(invariants, dict):
        ...
        invariants = {}
    if is_camera_ready_basic and "minimum_session_length" not in invariants:
        require_equal(errors, invariants.get("session_length_5_to_50"), True, "camera-ready session length flag")
        require_equal(errors, invariants.get("valid_test_items_subset_train"), True, "camera-ready tail seen flag")
    else:
