...
MODULE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "build_full_feature_dataset.py"
SPEC = importlib.util.spec_from_file_location("build_full_feature_dataset", MODULE_PATH)
...
def test_future_builder_emits_location_independent_metadata_natively() -> None:
    ...
    assert '"contract": "full-v5-location-independent-metadata-v1"' in source
